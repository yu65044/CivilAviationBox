...
var size;
var initFilter = {"groupOp":"AND","rules":[{"field":"compare_result","op":"exp","data":" compare_result = 0"}]};
var FilterJson;
	   		...
	   		success:function(data){
	   			...
	   			if(list.length>0){
		   			for (var i = 0; i < list.length; i++) {
						var user = list[i];
						var userImg = '/photo/file/no_head.jpg';
						var compName = compMap.get(user.compId) == null?"未知":compMap.get(user.compId);
						var orgName = compMap.get(user.compId) == null?"未知":orgMap.get(compOrgMap.get(user.compId));
						var sexNamae = sexMap.get(user.sex);
						var nationName = nationMap.get(user.nation);
						html += '<div class="col-sm-3" person-id="'+user.compareId+'">';
						...
						html += '                <h3><strong class="icon-view-text-overflow">'+user.name+'</strong></h3>';
						html += '                    <span class="icon-view-text-overflow">应聘企业名称：'+compName+'</span></br>';
						html += '                    <span class="icon-view-text-overflow">比中结果：'+user.compareVal+'</span></br>';
						html += '                    <span class="icon-view-text-overflow">比中时间：'+getSmpFormatDateByLong(user.compareDate,false)+'</span></br>';
						...
						html += '                    <span class="icon-view-text-overflow">所属机构 : '+orgName+'</span>';
						html += '            </div>';
	   			...
	   			Parent.children().each(function(){
	   				...
	   				$(this).dblclick(function(){
	   			    	...
	   			    	var examId = $(this).attr('person-id');
	   			    	openExamCompareDetail(examId);
		   			});
	   			})
