...
var DTIconTablePlug = {
    Initialize: function(params) {
    	...
    	this.OnLoad(PageIndex,PageSize,SameDayJson);
    },
    OnLoad:function(page,rows){
    	...
    	pageSize = rows;
    	$.ajax({
	   		url: ctx+'/zxgkWarnController/warnDatagrid.do?flag='+JSON.stringify(flag),
	   		data:{page:page,rows:rows,filterJson:JSON.stringify(SameDayJson),orgId:orgId},
	   		type:"post",
	   		dataType:"json",
	   		success:function(data){
	   			...
	   			if(list.length>0){
		   			for (var i = 0; i < list.length; i++) {
						...
						var monitorDeptName=user.monitorDeptName==''?"----":user.monitorDeptName;
						var feedbackType=user.feedbackType==null?"未反馈":"已反馈";
						...
						html +='                   <br><div class="m-t-xs font-bold">'+feedbackType+'</div>';
						html += '                </div>';
						html += '            </div>';
					...
					/*	html += '                    <span class="icon-view-text-overflow"><span>起飞机场名称: '+takeOffAirportName+'</span>';
						...
						html += '                    <span class="icon-view-text-overflow">责任单位: '+monitorDeptName+'</span>';
	   			...
	   			DTIconTablePlug.InitPagination(page,rows,data);
	   			DTIconTablePlug.OnMouseHover();
	   		}
	   	})
    },
    ReLoad: function(){
    	DTIconTablePlug.OnLoad(1,PageSize,SameDayJson);
    },
    InitPagination: function(page,rows,data){
    	if(data != null && data.total > 0){
	    	...
	    	for (var i = 0; i < PageList.length; i++) {
	    		html+='<li onclick="DTIconTablePlug.ChangePageSize('+PageList[i]+')"><a href="javascript:void(0)">'+PageList[i]+'</a></li>';
			}
	    	html+='	</ul>';
	    	html+='	</span> 条记录&nbsp;&nbsp;&nbsp;&nbsp;</span>';
	    	html+='</div>';
	    	html+='<div class="pull-right btn-group">';
	    	html+='    <button class="btn btn-white" type="button" onclick="DTIconTablePlug.OnLoad(1,'+PageSize+')">首页</button>';
	    	html+='    <button class="btn btn-white" type="button" onclick="DTIconTablePlug.PrePage()"><i class="fa fa-chevron-left"></i></button>';
	    	for (var i = 0; i < indexBtn.length; i++) {
	    		if(indexBtn[i]==pageIndex){
	    			...
	    			if(indexBtn[i]=='...'){
	    				...
	    				html+='<button class="btn btn-white" onclick="DTIconTablePlug.OnLoad('+indexBtn[i]+','+PageSize+')">'+indexBtn[i]+'</button>';
	    			}
	    		}
	    	}
	    	html+='    <button class="btn btn-white" type="button" onclick="DTIconTablePlug.NextPage('+totalPage+')"><i class="fa fa-chevron-right"></i></button>';
	    	html+='    <button class="btn btn-white" type="button" onclick="DTIconTablePlug.OnLoad('+totalPage+','+PageSize+')">尾页</button>';
	    	html+='</div>';
	    	html+='</div>';
    ...
    PrePage:function(){
    	...
    	if(pageIndex<=1){
    		if(PageIndex!=1) this.OnLoad(1,PageSize,SameDayJson);
    	}else{
    		this.OnLoad(pageIndex,PageSize,SameDayJson);
    	}
    },
    NextPage:function(t){
    	var pageIndex = PageIndex+1;
    	if(pageIndex>=t){
    		if(PageIndex!=t) this.OnLoad(t,PageSize,SameDayJson);
    	}else{
    		this.OnLoad(pageIndex,PageSize,SameDayJson);
    	}
    },
...
var ALLIconTablePlug = {
	    Initialize: function(params) {
	    	...
	    	this.OnLoad(PageIndex,PageSize,wholeJson);
	    },
	    OnLoad:function(page,rows){
	    	//console.log(FilterJson)
	    	PageIndex = page;
	    	...
	    	$.ajax({
		   		url: ctx+'/zxgkWarnController/warnDatagrid.do?flag='+JSON.stringify(flag),
		   		data:{page:page,rows:rows,filterJson:JSON.stringify(wholeJson),orgId:orgId},
		   		type:"post",
		   		dataType:"json",
		   		success:function(data){
		   			var list = data.rows;
		   			...
		   			Parent.children().each(function(){
		   				$(this).click(function(){
		   					ALLIconTablePlug.OnSelect(this);
			   			});
		   				$(this).dblclick(function(){
		   					$(this).siblings().children().css("border","1px solid #E7EAEC");
		   			    	$(this).children().css("border","1px solid #0098CB");
		   			...
		   			ALLIconTablePlug.InitPagination(page,rows,data);
		   			ALLIconTablePlug.OnMouseHover();
		   		}
		   	})
	    },
	    ReLoad: function(){
	    	ALLIconTablePlug.OnLoad(1,PageSize,wholeJson);
	    },
	    InitPagination: function(page,rows,data){
	    	if(data != null && data.total > 0){
		    	var total = parseInt(data.total);
		    	...
		    	html+='	<ul class="dropdown-menu" role="menu">';
		    	for (var i = 0; i < PageList.length; i++) {
		    		html+='<li onclick="ALLIconTablePlug.ChangePageSize('+PageList[i]+')"><a href="javascript:void(0)">'+PageList[i]+'</a></li>';
				}
		    	html+='	</ul>';
		    	html+='	</span> 条记录&nbsp;&nbsp;&nbsp;&nbsp;</span>';
		    	html+='</div>';
		    	html+='<div class="pull-right btn-group">';
		    	html+='    <button class="btn btn-white" type="button" onclick="ALLIconTablePlug.OnLoad(1,'+PageSize+')">首页</button>';
		    	html+='    <button class="btn btn-white" type="button" onclick="ALLIconTablePlug.PrePage()"><i class="fa fa-chevron-left"></i></button>';
		    	for (var i = 0; i < indexBtn.length; i++) {
		    		if(indexBtn[i]==pageIndex){
		    			html+='<button class="btn btn-white active">'+indexBtn[i]+'</button>';
		    		}else{
		    			if(indexBtn[i]=='...'){
		    				...
		    				html+='<button class="btn btn-white" onclick="ALLIconTablePlug.OnLoad('+indexBtn[i]+','+PageSize+')">'+indexBtn[i]+'</button>';
		    			}
		    		}
		    	}
		    	html+='    <button class="btn btn-white" type="button" onclick="ALLIconTablePlug.NextPage('+totalPage+')"><i class="fa fa-chevron-right"></i></button>';
		    	html+='    <button class="btn btn-white" type="button" onclick="ALLIconTablePlug.OnLoad('+totalPage+','+PageSize+')">尾页</button>';
		    	html+='</div>';
		    	html+='</div>';
		    	$(document).find('#pagination').remove();
		    	$(Parent).after(html);
	    ...
	    PrePage:function(){
	    	var pageIndex = PageIndex-1;
	    	if(pageIndex<=1){
	    		if(PageIndex!=1) this.OnLoad(1,PageSize,wholeJson);
	    	}else{
	    		this.OnLoad(pageIndex,PageSize,wholeJson);
	    	}
	    },
	    NextPage:function(t){
	    	var pageIndex = PageIndex+1;
	    	if(pageIndex>=t){
	    		if(PageIndex!=t) this.OnLoad(t,PageSize,wholeJson);
	    	}else{
	    		this.OnLoad(pageIndex,PageSize,wholeJson);
	    	}
	    },
