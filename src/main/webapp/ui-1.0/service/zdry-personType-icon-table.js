...
var PageIndex = 1;
var PageSize = 12;
var personTypeSelectUrl = ctx + '/zdryPersonTypeController/personTypeInfo.do';
var size;
...
var IconTablePlug = {
	   		...
	   		success:function(data){
	   			...
	   			if(list.length>0){
		   			for (var i = 0; i < list.length; i++) {
						var personType = list[i];
						html += '<div class="col-sm-3" person-id="'+personType.personTypeId+'">';
						html += '    <div class="contact-box" id="icon-item">';
						html += '        <a href="javascript:void(0);" >';
						html += '            <div class="col-sm-8">';
						html += '                <h3 class="text-overflow"><strong>'+personType.personName+'</strong></h3>';
						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">证件号码：'+personType.personCardNumber+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">其他证件号码1：'+personType.personOtherCard1+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">其他证件号码2：'+personType.personOtherCard2+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">布控单位(名称) : '+personType.monitorDeptName+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">布控人姓名: '+personType.monitorPersonName+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">布控人手机号: '+personType.monitorPersonPhone+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">布控开始时间: '+getSmpFormatDateByLong(personType.monitorTime, true, false)+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">创建时间: '+getSmpFormatDateByLong(personType.createTime, true, false)+'</span>';
						html += '            </div>';
	   			...
	   			Parent.children().each(function(){
	   				...
	   				$(this).dblclick(function(){
	   			    	...
	   			    	var personTypeId = $(this).attr('person-id');
	   					IconTablePlug.DblClick(personTypeSelectUrl,personTypeId);
		   			});
	   			})
	...
	DblClick: function(url,personTypeId){
    	var index = layer.open({
			   type:2,
			   area: [width+'px', size.height-400+'px'],  
			   title:"详细信息",
			   skin: 'layer-win-class',
			   fix: true,
			   maxmin: false,//窗口最大最小
			   closeBtn:1, //0:不显示关闭按钮，1：显示关闭按钮，2：关闭按钮在右正上方
			   content: url+'?personTypeId='+personTypeId,
		});
	},
