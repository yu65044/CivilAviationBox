...
var PageIndex = 1;
var PageSize = 12;
var personPoliceSelectUrl = ctx + '/manageController/personDetail.do';
var size;
	   		...
	   		success:function(data){
	   			...
	   			if(list.length>0){
		   			for (var i = 0; i < list.length; i++) {
						...
						if(personPolice.photoUrl == null || personPolice.photoUrl == ''){
							userImg = '/static/images/no_head.jpg';
						}else{
							userImg = personPolice.photoUrl;
						}
						var personName=ifNull(personPolice.personName);
						var personCardNumber=ifNull(personPolice.personCardNumber);
						var monitorDeptName=ifNull(personPolice.monitorDeptName);
						var monitorPersonName=ifNull(personPolice.monitorPersonName);
						var monitorPersonPhone=ifNull(personPolice.monitorPersonPhone);
						var monitorTime=ifNull(getSmpFormatDateByLong(personPolice.monitorTime, false, false));
						var monitorTimeEnd=ifNull(getSmpFormatDateByLong(personPolice.monitorTimeEnd, false, false));
						...
						html += '                    <div class="m-t-xs font-bold text-overflow">'+personName+'</div>';
						html += '                </div>';
						html += '            </div>';
						html += '            <div class="col-sm-8">';
//						html += '                <h3 class="text-overflow"><strong>'+personPolice.personName+'</strong></h3>';
						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">身份证号：'+personCardNumber+'</span>';
...
//						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">其他证件号码2：'+personPolice.personOtherCard2+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">责任单位 : '+monitorDeptName+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">责任人姓名: '+monitorPersonName+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">责任人手机号: '+monitorPersonPhone+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">布控开始时间: '+monitorTime+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">布控结束时间: '+monitorTimeEnd+'</span>';
						html += '            </div>';
