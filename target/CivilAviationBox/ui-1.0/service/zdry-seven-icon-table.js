...
var PageIndex = 1;
var PageSize = 12;
var personSelectUrl = ctx+'/zdrySevenController/detailInfo.do';
var size;
var initFilter = {"groupOp":"AND","rules":[{"field":"yxx","op":"exp","data":" yxx = 1"}]};
var FilterJson;
var width = 900;//弹窗的宽度
var height = 600;//弹窗的高度
var IconTablePlug = {
	   		...
	   		success:function(data){
	   			...
	   			if(list.length>0){
		   			for (var i = 0; i < list.length; i++) {
						...
						var userSex = sexMap.get(user.xb) == null?"----":sexMap.get(user.xb);
						var userNation = nationMap.get(user.mz) == null?"----":nationMap.get(user.mz);
						html += '<div class="col-sm-3" person-id="'+user.id+'">';
						...
						html += '                    <div class="m-t-xs font-bold text-overflow">'+user.xm+'</div>';
						html += '                </div>';
						html += '            </div>';
						html += '            <div class="col-sm-8">';
						html += '                <h3 class="text-overflow"><strong>'+(user.xmpy==''?user.xm:user.xmpy)+'</strong></h3>';
						...
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">身份证号 : '+(user.sfzh==''?'----':user.sfzh)+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">其它证件号 : '+(user.qtzjhm==''?'----':user.qtzjhm)+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">户籍地址 : '+(user.hjdxz==''?'----':user.hjdxz)+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">现住地址 : '+(user.xzdxz==''?'----':user.xzdxz)+'</span>';
						html += '            </div>';
