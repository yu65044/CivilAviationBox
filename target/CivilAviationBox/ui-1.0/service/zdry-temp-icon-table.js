...
var PageIndex = 1;
var PageSize = 12;
var eppSelectUrl = ctx + '/zdryTempController/eppInfo.do';
var size;
	   		...
	   		success:function(data){
	   			...
	   			if(list.length>0){
		   			for (var i = 0; i < list.length; i++) {
						var epp = list[i];
						html += '<div class="col-sm-3" person-id="'+epp.personId+'">';
						html += '    <div class="contact-box" id="icon-item">';
						html += '        <a href="javascript:void(0);" >';
						html += '            <div class="col-sm-8">';
						html += '                <h3 class="text-overflow"><strong>'+epp.personName+'</strong></h3>';
						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">证件号码：'+epp.personCardNumber+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">其他证件号码1：'+epp.personOtherCard1+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">其他证件号码2：'+epp.personOtherCard2+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;font-size:10px;float:left; padding-top:5px;">布控单位(名称) : '+epp.monitorDeptName+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">布控人姓名: '+epp.monitorPersonName+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">布控人手机号: '+epp.monitorPersonPhone+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">布控开始时间: '+getSmpFormatDateByLong(epp.monitorTime, true, false)+'</span>';
						html += '                    <span class="text-overflow" style="width:100%;float:left;font-size:10px;padding-top:5px;">创建时间: '+getSmpFormatDateByLong(epp.createTime, true, false)+'</span>';
						html += '            </div>';
	   			...
	   			Parent.children().each(function(){
	   				...
	   				$(this).dblclick(function(){
	   					...
	   					IconTablePlug.DblClick(eppSelectUrl,personId);
		   			});
	   			})
