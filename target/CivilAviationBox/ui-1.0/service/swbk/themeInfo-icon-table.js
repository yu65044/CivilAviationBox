...
var IconTablePlug = {
    ...
    OnLoad:function(page,rows){
    	...
    	pageSize = rows;
    	$.ajax({
	   		url: ctx+'/swbkController/themeInfoDatagrid.do',
	   		...
	   		success:function(data){
	   			...
	   			if(list.length>0){
		   			for (var i = 0; i < list.length; i++) {
						...
						var themeName = user.compId == null?"----":user.compId;
						var remarks = user.remarks == null?"----":user.remarks;
						var createUser = user.createUser == null?"----":user.createUser;
						html += '<div class="col-sm-3" org-id="'+user.orgId+'" id="'+user.id+'">';
						...
						html += '                    <div class="m-t-xs font-bold">'+user.themeName+'</div>';
						html += '                </div>';
						html += '            </div>';
						html += '            <div class="col-sm-8">';
						html += '                <h3><strong class="text-overflow">'+user.themeName+'</strong></h3>';
						html += '                    <span class="icon-view-text-overflow">备注：'+remarks+'</span>';
						html += '                    <span class="icon-view-text-overflow">创建人：'+user.createUser+'</span>';
						html += '            </div>';
